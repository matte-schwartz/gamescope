...
> For detailed build instructions, including build requirements and distribution-specific package lists, check the wiki
...
When running gamescope from a virtual terminal (TTY), you can use gamescope's DRM backend to achieve a Steam Deck-like experience on other devices. The easiest way to do this is by running the entire Steam client within gamescope. To enable Steam integration, add `-e` or `--steam` to your launch arguments for gamescope.
...
# Run Steam using the Steam Deck client with mangoapp and full Steam client integration within gamescope
# Note: the Steam Deck client is only meant to be run on SteamOS 3 at this time
gamescope -e --mangoapp -- steam -steamdeck -steamos3 -steampal -gamepadui
```
...
Gamescope offers a wide range of configuration options for HDR support in both a nested usecase with the Wayland backend or in an embedded usecase with the DRM backend. If your setup supports HDR, enabling HDR support within gamescope is as simple as adding `--hdr-enabled` to your gamescope launch options. 

If you want to check if HDR is working, some `--mangoapp` overlays show HDR status, or on the right-side performance menu of Steam's GamepadUI next to the sliders for volume. Gamescope's logging will also tell you about the metadata of the HDR content that it is presenting.
...
To print your gamescope version and display information when filing an issue, you can run `gamescopectl` without any arguments while gamescope is running.
...
Generally, convars will be set with 0 or false for disabled, and 1 or true for enabled. Some convars like logging level will have additional values as noted in `gamescopectl help`.
...
Some convars must be set in code before building gamescope. `gamescopectl help` lists every possible convar, not the just the ones that can be toggled while gamescope is running.
