...
// Whether mangoapp would show its overlay for this config file and MANGOHUD_CONFIG
// environment, read the way mangoapp reads them. A key with no value reads as 1
// and '#' starts a comment. A non-empty environment replaces the file unless it
// sets read_cfg, then it is applied on top. An explicit no_display beats the
// preset and preset 0 hides.
