...
gamescope.config.known_displays.legion_go_s_lcd = {
    ...
    -- and the panel is not responsive to tuning VFPs. The VRR limiter covers the full
    -- range of the panel so this covers the non-VRR usecase.
    dynamic_refresh_rates = legion_go_s_lcd_refresh_rates,
    dynamic_modegen = function(base_mode, refresh)
        debug("Generating mode "..refresh.."Hz for Legion Go S LCD")
    ...
    matches = function(display)
        ...
        for index,value in ipairs(lcd_types) do
            if value.vendor == display.vendor and value.model == display.model and value.product == display.product
                then
                debug("[legion_go_s_lcd] Matched vendor: "..display.vendor.." model: "..display.model.." product: "..display.product)
                return 5000
            end
        end
